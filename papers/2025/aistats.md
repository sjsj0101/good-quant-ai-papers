<!-- Generated from data/papers.yaml and data/coverage.yaml by scripts/render.py. Do not edit directly. -->

# AISTATS 2025

**Coverage status:** Pending

**Checked on:** 2026-07-12

**Cataloged eligible papers:** 1

**Tracks checked:** Main

**Tracks pending:** None

**Official audit sources:** [Source 1](<https://proceedings.mlr.press/v258/>) · [Source 2](<https://github.com/mlresearch/v258/tree/f9b46f555d3501fe4cc7e31329af9ab55a7b66c6/_posts>) · [Source 3](<https://aistats.org/aistats2025/>)

**Coverage notes:** The immutable tree enumerates 583 main records. Nineteen official blobs were screened through the allowed connector, one verified paper was cataloged, and 564 title and abstract records remain unscreened. The existence and rosters of any side programs are also unresolved; main is not complete.

1 verified papers curated for direct relevance to quantitative finance and asset management. Tracks are separated so main-conference, workshop, position, and affinity status remains visible.

[← Back to the main index](../../README.md)

## Main Conference (1)

### [Approximate Equivariance in Reinforcement Learning](<https://openreview.net/forum?id=SOrZWcuz3j>)

**Catalog ID:** 2025-aistats-park-approximate-equivariance

**Authors:** Jung Yeon Park, Sujay Bhatt, Sihan Zeng, Lawson L.S. Wong, Alec Koppel, Sumitra Ganesh, Robin Walters

**Venue / year:** AISTATS · 2025

**Track / presentation:** Main · Not Specified

**Status / verified:** Published · 2026-07-12

**Topics:** Alpha Modeling · Financial Agents · Evaluation

**Assets / frequency:** Equities

**Identifiers:** OpenReview SOrZWcuz3j

**Focus:** Relaxes exact symmetry in reinforcement-learning policies and evaluates the resulting architecture on stock trading with real financial data.

**Why it matters:** Tests whether approximate market symmetries improve trading-policy performance and noise robustness without imposing unrealistic invariances.

**Links:** [Official venue record](<https://proceedings.mlr.press/v258/park25d.html>) · [Paper](<https://openreview.net/forum?id=SOrZWcuz3j>)

---
